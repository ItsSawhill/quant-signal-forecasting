...
def run_backtest(portfolio_frame: pd.DataFrame, transaction_cost_bps: float = 5.0) -> tuple[pd.DataFrame, dict[str, float]]:
    """Run a daily rebalanced long/short backtest."""
    frame = portfolio_frame.copy().sort_values(["date", "asset"])
    frame["asset_pnl"] = frame["position"] * frame["realized_return"]
...
def save_backtest_outputs(daily_returns: pd.DataFrame, metrics: dict[str, float], prefix: str) -> tuple[Path, Path, Path]:
    ...
    ax.plot(daily_returns["date"], daily_returns["equity_curve"])
